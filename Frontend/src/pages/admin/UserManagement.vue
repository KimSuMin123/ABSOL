<template>
  <q-page class="q-pa-md bg-grey-2">
    ...
    <div class="row q-col-gutter-md">
      <div v-if="users.length === 0 && !loading" class="col-12 text-center q-pa-xl text-grey-6 bg-white shadow-1 rounded-borders">조회된 회원 내역이 없습니다.</div>
        ...
        <q-card flat bordered class="user-card shadow-1">
          <q-card-section horizontal>
            ...
            <q-card-section class="col-4 q-py-md border-left">
              ...
              <div class="text-caption text-grey-7 q-mt-sm ellipsis-2-lines"><q-icon name="location_on" size="xs" /> {{ user.address || '주소 미등록' }}</div>
            </q-card-section>
            ...
            <q-card-section class="col-3 q-py-md flex flex-center border-left bg-grey-1">
              <div class="text-center full-width q-gutter-y-xs">
                ...
                <q-separator />
                <q-btn flat color="grey-8" label="기본 정보 수정" icon="edit" dense class="full-width" @click="openEditDialog(user)" />
              </div>
            </q-card-section>
          </q-card-section>
        </q-card>
      ...
      <q-card style="min-width: 400px">
        ...
        <q-card-section class="q-pa-md">
          <q-form @submit="updateUser" class="q-gutter-md">
            ...
            <q-select v-model="editForm.level" :options="['Basic', 'Standard', 'Green', 'Silver', 'Gold']" label="회원 등급" outlined dense />
    ...
    <AdminMyPCModal 
      v-model="pcModal.show" 
      :user-id="pcModal.userId" 
      :customer-name="pcModal.userName" 
    />
...
// 컴포넌트 임포트 (경로를 본인의 프로젝트 구조에 맞게 수정하세요)
...
const editForm = ref({ user_id: null, customer_name: '', phone: '', address: '', level: '' });
...
const updateUser = async () => {
  try {
    const res = await axios.patch(`http://localhost:3000/api/admin/users/${editForm.value.user_id}`, editForm.value);
    if (res.data.success) {
      $q.notify({ color: 'positive', message: '수정 완료' });
