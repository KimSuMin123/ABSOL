...
const loadUserPcData = async () => {
  loading.value = true;
  try {
    const res = await axios.get(`https://port-0-absol-mk2l6v1wd9132c30.sel3.cloudtype.app/api/mypc/user/${props.userId}`);
    if (res.data.success) {
...
const savePc = async (pc) => {
  try {
    const res = await axios.post(`https://port-0-absol-mk2l6v1wd9132c30.sel3.cloudtype.app/api/mypc/user/${props.userId}`, pc);
    if (res.data.success) {
...
const confirmDelete = (pc, index) => {
  ...
  $q.dialog({
    ...
    try {
      await axios.delete(`https://port-0-absol-mk2l6v1wd9132c30.sel3.cloudtype.app/api/mypc/${pc.mypc_id}`);
