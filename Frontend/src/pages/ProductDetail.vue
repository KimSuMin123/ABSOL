    ...
    <div class="preview-side">
      <div class="preview-wrapper">
        <div id="pdf-area" ref="pdfArea" class="quote-paper">
          ...
          <div v-if="form.pay_method === '미결제' && firstProductId" class="q-mt-md q-pa-sm" style="border: 1px dashed #1a237e; background: #f0f4ff;">
            <div style="font-size: 11px; color: #1a237e; font-weight: bold;">[온라인 결제 안내]</div>
            <div style="font-size: 11px; color: #333; word-break: break-all;">
              주소: <strong>http://localhost:5173/product/{{ firstProductId }}</strong>
            </div>
            <div style="font-size: 10px; color: #666;">* 위 주소에 접속하시면 카드 결제 및 주문 확정이 가능합니다.</div>
          </div>
...
const submitOfflineOrder = async () => {
  ...
  try {
    // 1. 제품 먼저 등록하여 ID 확보
    const validItems = activeItems.value;
    ...
    if (validItems.length > 0) {
      ...
      registeredProductIds = responses.map(res => res.data.data?.product_id || res.data.data?.id);
      firstProductId.value = registeredProductIds[0];
    }
    ...
    orderFormData.append('data', JSON.stringify({ 
      ...
      is_paid: isUnpaid ? 0 : 1,
      state: isUnpaid ? '접수완료' : '결제완료'
    }));
    ...
    console.error(err);
    alert('처리 중 에러 발생: ' + (err.response?.data?.message || err.message));
  } finally {
